...
class Game:
    ...
    def __init__(self):
        ...
        self.items = Items('items_data.txt')
            ...
            #NEWSTUFF: creates player object
        ...
        self.verbs = {'climb', 'close', 'dig', 'drop', 'examine', 'go',
            ...
            'swim', 'take', 'touch', 'type', 'unlock', 'break', 'eat'}
    ...
    def process_command(self, command):
        ...
        if verb == 'go':
            ...
            self.process_break(words)
        else:
            print("I don't know how to do that.")
    
    # Make sure you cover all scenarios see process_go for
    #reference
    # Command methods ================================
    def process_eat(self, words):
        # NEWSTUFF: Handles eating the bread
        if len(words) == 1:
            print('You have to tell me what you want to eat.')
        elif  'bread' not in self.player.inventory:
            print("There's nothing to eat.")
        elif 'bread' in self.player.inventory:
            print('You ate moldy bread. Yuck!')

        # NEWSTUFF: Health decrease
            self.player.health -= 5
            self.player.inventory.remove('bread')
        else:
            print('You are out of bread')
         #NEWSTUFF: Break method   
    def process_break(self, words):
        mirror = self.items.get_item('mirror')
        if len(words) == 1:
            print('What do you want to break.')
        elif words[1] != 'mirror':
            print('You cannot break that.')
        elif self.current_room != 'bedroom':
            print('You must be in the bedroom.')
        elif 'spoon' not in self.player.inventory and 'wrench' not in self.player.inventory:
            print('Improper tools')
        elif mirror.status == 'broken':
            print('The mirror is already broken.')
        else:
        
            if 'spoon' in self.player.inventory or 'wrench' in self.player.inventory:
                print('You broke the mirror. 7 years bad luck!')
                self.player.health -= 5
                mirror.status = 'broken'
                mirror.description = 'A broken mirror.'
            else:
                print('Improper tools')
    ...
    def process_take(self, words):
        if len(words) == 1:
            ...
            if item.name in room.items_in_room:
                ...
                elif self.items.get_total_weight(self.player.inventory) + item.weight > self.player.max_capacity:
                ...
                for item_name_in_room in room.items_in_room:
                    if item_name_in_room in self.containers:
                        ...
                        if item.name in container.contents and container.status == 'open':
                            if self.items.get_total_weight(self.player.inventory) + item.weight > 5:
                                print("You can't carry it with your current load.")
                            else:
                                self.player.inventory.add(item.name)
    ...
    def process_swim(self):
        #NEWSTUFF: Player health depends if they can swim or not
        if self.player.health < 10:
            print('You do not have the strength to swim.')
        else:
            room = self.rooms.get_room(self.current_room)
    ...
    def process_dig(self):
        #NEWSTUFF: Health implementation for dig
        if self.player.health < 10:
            print('You do not have the strength to dig.')
        else:
